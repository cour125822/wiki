...
用带 `tz` 参数的纪元时间戳创建 [`Timestamp`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.Timestamp.html#pandas.Timestamp) 或 [`DatetimeIndex`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.DatetimeIndex.html#pandas.DatetimeIndex) 时，要先把纪元时间戳转化为 UTC，然后再把结果转换为指定时区。不过这种操作方式现在已经[废弃open in new window](https://pandas.pydata.org/pandas-docs/stable/whatsnew/v0.24.0.html#whatsnew-0240-deprecations-integer-tz)了，对于其它时区 Wall Time 里的纪元时间戳，建议先把纪元时间戳转换为无时区时间戳，然后再把时区本地化。
...
### [#](https://www.pypandas.cn/docs/user_guide/timeseries.html#局部字符串索引)局部字符串索引
...
`DateOffset` 基础操作类似于 `dateutil.relativedelta`（[relativedelta 文档open in new window](https://dateutil.readthedocs.io/en/stable/relativedelta.html)），可按指定的日历日时间段偏移日期时间。可用算数运算符（+）或 `apply` 方法执行日期偏移操作。
...
| [`QuarterBegin`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.tseries.offsets.QuarterBegin.html#pandas.tseries.offsets.QuarterBegin) |      `'QS'`       |                 日历日季初                 |
...
节日日历支持节假日列表。更多信息，请参阅[节日日历open in new window](https://pandas.pydata.org/pandas-docs/stable/user_guide/timeseries.html#timeseries-holiday)文档。
...
### [#](https://www.pypandas.cn/docs/user_guide/timeseries.html#别名组合)别名组合
...
## [#](https://www.pypandas.cn/docs/user_guide/timeseries.html#时间序列实例方法)时间序列实例方法

### [#](https://www.pypandas.cn/docs/user_guide/timeseries.html#移位与延迟)移位与延迟

有时，需要整体向前或向后移动时间序列里的值，这就是移位与延迟。实现这一操作的方法是 [`shift()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.Series.shift.html#pandas.Series.shift)，该方法适用于所有 pandas 对象。
...
`shift` 方法支持 `freq` 参数，可以把 `DateOffset`、`timedelta` 对象、[`偏移量别名`open in new window](https://pandas.pydata.org/pandas-docs/stable/user_guide/timeseries.html#timeseries-offset-aliases) 作为参数值：
...
除更改数据与索引的对齐方式外，`DataFrame` 与 `Series` 对象还提供了 [`tshift()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.Series.tshift.html#pandas.Series.tshift) 便捷方法，可以指定偏移量修改索引日期。
...
### [#](https://www.pypandas.cn/docs/user_guide/timeseries.html#频率转换)频率转换

改变频率的函数主要是 [`asfreq()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.Series.asfreq.html#pandas.Series.asfreq)。对于 `DatetimeIndex`，这就是一个调用 [`reindex()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.Series.reindex.html#pandas.Series.reindex)，并生成 `date_range` 的便捷打包器。
...
### [#](https://www.pypandas.cn/docs/user_guide/timeseries.html#向前与向后填充)向前与向后填充

与 `asfreq` 与 `reindex` 相关的是 [`fillna()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.Series.fillna.html#pandas.Series.fillna)，有关文档请参阅[缺失值open in new window](https://pandas.pydata.org/pandas-docs/stable/user_guide/missing_data.html#missing-data-fillna)。

### [#](https://www.pypandas.cn/docs/user_guide/timeseries.html#转换-python-日期与时间)转换 Python 日期与时间

用 `to_datetime` 方法可以把`DatetimeIndex` 转换为 Python 原生 [`datetime.datetime`open in new window](https://docs.python.org/3/library/datetime.html#datetime.datetime) 对象数组。

## [#](https://www.pypandas.cn/docs/user_guide/timeseries.html#重采样)重采样

警告

0.18.0 版修改了 `.resample` 接口，现在的 `.resample` 更灵活，更像 groupby。参阅[更新文档open in new window](https://pandas.pydata.org/pandas-docs/stable/whatsnew/v0.18.0.html#whatsnew-0180-breaking-resample) ，对比新旧版本操作的区别。

Pandas 有一个虽然简单，但却强大、高效的功能，可在频率转换时执行重采样，如，将秒数据转换为 5 分钟数据，这种操作在金融等领域里的应用非常广泛。

[`resample()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.Series.resample.html#pandas.Series.resample) 是基于时间的分组操作，每个组都遵循归纳方法。参阅 [Cookbook 示例open in new window](https://pandas.pydata.org/pandas-docs/stable/user_guide/cookbook.html#cookbook-resample)了解高级应用。

从 0.18.0 版开始，`resample()` 可以直接用于 `DataFrameGroupBy` 对象，参阅 [groupby 文档open in new window](https://pandas.pydata.org/pandas-docs/stable/user_guide/groupby.html#groupby-transform-window-resample)。

注意

`.resample()` 类似于基于时间偏移量的 [`rolling()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.Series.rolling.html#pandas.Series.rolling) 操作，请参阅[这里open in new window](https://pandas.pydata.org/pandas-docs/stable/user_guide/computation.html#stats-moments-ts-versus-resampling)的讨论。

### [#](https://www.pypandas.cn/docs/user_guide/timeseries.html#基础知识)基础知识
...
任何支持[派送（dispatch）open in new window](https://pandas.pydata.org/pandas-docs/stable/user_guide/groupby.html#groupby-dispatch)的函数都可用于 `resample` 返回对象，包括 `sum`、`mean`、`std`、`sem`、`max`、`min`、`mid`、`median`、`first`、`last`、`ohlc`：
...
这种操作可能会导致时间回溯，即后面的时间会被拉回到前面的时间，如下例的 [`BusinessDay`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.tseries.offsets.BusinessDay.html#pandas.tseries.offsets.BusinessDay) 频率所示。
...
### [#](https://www.pypandas.cn/docs/user_guide/timeseries.html#上采样)上采样
...
### [#](https://www.pypandas.cn/docs/user_guide/timeseries.html#稀疏重采样)稀疏重采样
...
### [#](https://www.pypandas.cn/docs/user_guide/timeseries.html#聚合)聚合

类似于[聚合 APIopen in new window](https://pandas.pydata.org/pandas-docs/stable/getting_started/basics.html#basics-aggregate)，[Groupby APIopen in new window](https://pandas.pydata.org/pandas-docs/stable/user_guide/groupby.html#groupby-aggregate) 及[窗口函数 APIopen in new window](https://pandas.pydata.org/pandas-docs/stable/user_guide/computation.html#stats-aggregate)，`Resampler` 可以有选择地重采样。
...
### [#](https://www.pypandas.cn/docs/user_guide/timeseries.html#分组迭代)分组迭代

`Resampler`对象迭代分组数据的操作非常自然，类似于 [`itertools.groupby()`open in new window](https://docs.python.org/3/library/itertools.html#itertools.groupby)：
...
了解更多详情，请参阅[分组迭代open in new window](https://pandas.pydata.org/pandas-docs/stable/user_guide/groupby.html#groupby-iterating-label)或 [`itertools.groupby()`open in new window](https://docs.python.org/3/library/itertools.html#itertools.groupby)。

## [#](https://www.pypandas.cn/docs/user_guide/timeseries.html#时间跨度表示)时间跨度表示

规律时间间隔可以用 pandas 的 `Peirod` 对象表示，`Period` 对象序列叫做 `PeriodIndex`，用便捷函数 `period_range` 创建。

### [#](https://www.pypandas.cn/docs/user_guide/timeseries.html#period)Period
...
### [#](https://www.pypandas.cn/docs/user_guide/timeseries.html#periodindex-与-period-range)PeriodIndex 与 period_range
...
`PeriodIndex` 有自己的数据类型，即 `period`，请参阅 [Period 数据类型open in new window](https://pandas.pydata.org/pandas-docs/stable/user_guide/timeseries.html#timeseries-period-dtype)。

### [#](https://www.pypandas.cn/docs/user_guide/timeseries.html#period-数据类型)Period 数据类型

*0.19.0 版新增*。

`PeriodIndex` 的自定义数据类型是 `period`，是 pandas 扩展数据类型，类似于[带时区信息的数据类型open in new window](https://pandas.pydata.org/pandas-docs/stable/user_guide/timeseries.html#timeseries-timezone-series)（`datetime64[ns, tz]`）。

`Period` 数据类型支持 `freq` 属性，还可以用 `period[freq]` 表示，如，`period[D]` 或 `period[M]`，这里用的是[频率字符串open in new window](https://pandas.pydata.org/pandas-docs/stable/user_guide/timeseries.html#timeseries-offset-aliases)。
...
### [#](https://www.pypandas.cn/docs/user_guide/timeseries.html#periodindex-局部字符串索引)PeriodIndex 局部字符串索引

与 `DatetimeIndex` 一样，`PeriodIndex` 可以把日期与字符串传递给 `Series` 与 `DataFrame`。详情请参阅 [DatetimeIndex 局部字符串索引open in new window](https://pandas.pydata.org/pandas-docs/stable/user_guide/timeseries.html#timeseries-partialindexing)。
...
### [#](https://www.pypandas.cn/docs/user_guide/timeseries.html#频率转换与-periodindex-重采样)频率转换与 `PeriodIndex` 重采样
...
### [#](https://www.pypandas.cn/docs/user_guide/timeseries.html#不同表现形式之间的转换)不同表现形式之间的转换
...
## [#](https://www.pypandas.cn/docs/user_guide/timeseries.html#界外跨度表示)界外跨度表示

数据在 `Timestamp` 限定边界外时，参阅 [Timestamp 限制open in new window](https://pandas.pydata.org/pandas-docs/stable/user_guide/timeseries.html#timeseries-timestamp-limits)，可以用 `PeriodIndex` 或 `Periods` 的 `Series` 执行计算。
...
## [#](https://www.pypandas.cn/docs/user_guide/timeseries.html#时区控制)时区控制

利用 `pytz` 与 `datetuil` 或标准库 `datetime.timezone` 对象，pandas 能以多种方式处理不同时区的时间戳。

### [#](https://www.pypandas.cn/docs/user_guide/timeseries.html#处理时区)处理时区
...
用 [`date_range()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.date_range.html#pandas.date_range)、[`Timestamp`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.Timestamp.html#pandas.Timestamp) 、[`DatetimeIndex`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.DatetimeIndex.html#pandas.DatetimeIndex) 的 `tz_localize` 方法或 `tz` 关键字参数，可以为这些日期加上本地时区，即，把指定时区分配给不带时区的日期。还可以传递 `pytz` 、 `dateutil` 时区对象或奥尔森时区数据库字符串。奥尔森时区字符串默认返回 `pytz` 时区对象。要返回 `dateutil` 时区对象，在字符串前加上 `datetuil/`。
...
使用 `pytz` 时区时，对于相同的输入时区，[`DatetimeIndex`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.DatetimeIndex.html#pandas.DatetimeIndex) 会构建一个与 [`Timestamp`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.Timestamp.html#pandas.Timestamp) 不同的时区对象。[`DatetimeIndex`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.DatetimeIndex.html#pandas.DatetimeIndex) 具有一组 [`Timestamp`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.Timestamp.html#pandas.Timestamp) 对象，UTC 偏移量也不同，不能用一个 `pytz` 时区实例简洁地表示，[`Timestamp`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.Timestamp.html#pandas.Timestamp) 则可以用来指定 UTC 偏移量表示一个时点。
...
注意不同版本时区支持库对时区的定义并不一致。在处理本地存储数据时使用一种版本的支持库，在运算时使用另一种版本的支持库，可能会引起问题。参阅[本文open in new window](https://pandas.pydata.org/pandas-docs/stable/user_guide/io.html#io-hdf5-notes)了解如何处理这种问题。
...
在后台，所有 Timestamp 都存储为 UTC。含时区信息的 [`DatetimeIndex`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.DatetimeIndex.html#pandas.DatetimeIndex) 或 [`Timestamp`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.Timestamp.html#pandas.Timestamp) 的值有其自己的本地化时区字段（日、小时、分钟等）。不过，对于不同时区时间戳，如果其 UTC 值相同，将被视作是相等的时间。
...
不同时区 [`Series`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.Series.html#pandas.Series) 之间的操作生成的是与 UTC 时间戳数据对齐的 UTC [`Series`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.Series.html#pandas.Series)。
...
### [#](https://www.pypandas.cn/docs/user_guide/timeseries.html#本地化导致的混淆时间)本地化导致的混淆时间
...
### [#](https://www.pypandas.cn/docs/user_guide/timeseries.html#本地化时不存在的时间)本地化时不存在的时间
...
### [#](https://www.pypandas.cn/docs/user_guide/timeseries.html#时区序列操作)时区序列操作

无时区 [`Series`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.Series.html#pandas.Series) 值的数据类型是 datetime64[ns]。
...
有时区 [`Series`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.Series.html#pandas.Series) 值的数据类型是 datetime64[ns, tz]，`tz` 指的是时区。
...
这两种 [`Series`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.Series.html#pandas.Series) 的时区信息都可以用 `.dt` 访问器操控，参阅 [dt 访问器open in new window](https://pandas.pydata.org/pandas-docs/stable/getting_started/basics.html#basics-dt-accessors)。
...
在 `Series` 上应用 [`Series.to_numpy()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.Series.to_numpy.html#pandas.Series.to_numpy)，返回数据的 NumPy 数组。虽然 NumPy 可以**输出**本地时区！但其实它当前并不支持时区，因此，有时区时间戳数据返回的是时间戳对象数组：
