...
description: 
published: true
date: 2023-03-15T03:23:05.040Z
tags: 
editor: markdown
dateCreated: 2023-03-15T03:23:03.141Z
---

# 多层级索引和高级索引

本章节包含了[使用多层级索引](https://www.pypandas.cn/docs/user_guide/advanced.html#advanced-hierarchical) 以及 [其他高级索引特性](https://www.pypandas.cn/docs/user_guide/advanced.html#indexing-index-types)。

请参阅 [索引与选择数据](https://www.pypandas.cn/docs/user_guide/indexing.html#indexing)来获得更多的通用索引方面的帮助文档

警告

基于实际的使用场景不同，返回的内容也会不尽相同（返回一个数据的副本，或者返回数据的引用）。有时，这种情况被称作连锁赋值，但是这种情况应当被尽力避免。参见[返回视图or返回副本](https://www.pypandas.cn/docs/user_guide/indexing.html#indexing-view-versus-copy)。

参见 [cookbook](https://www.pypandas.cn/document/cookbook/index.html)，获取更多高级的使用技巧。

## [#](https://www.pypandas.cn/docs/user_guide/advanced.html#分层索引-多层级索引)分层索引（多层级索引）

分层/多级索引在处理复杂的数据分析和数据操作方面为开发者奠定了基础，尤其是在处理高纬度数据处理上。本质上，它使您能够在较低维度的数据结构(如 `Series`(1d)和`DataFrame` (2d))中存储和操作任意维数的数据。

在本节中，我们将展示“层次”索引的确切含义，以及它如何与上面和前面部分描述的所有panda索引功能集成。稍后，在讨论[分组](https://www.pypandas.cn/docs/user_guide/groupby.html#groupby) 和[数据透视与重塑性数据](https://www.pypandas.cn/docs/user_guide/reshaping.html#reshaping)时，我们将展示一些重要的应用程序，以说明它如何帮助构建分析数据的结构。

请参阅[cookbook](https://www.pypandas.cn/docs/user_guide/cookbook.html#cookbook-multi-index)，查看一些高级策略.

*在0.24.0版本中的改变:*`MultIndex.labels`被更名为[`MultiIndex.codes`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.MultiIndex.codes.html#pandas.MultiIndex.codes) ，同时 `MultiIndex.set_labels` 更名为 [`MultiIndex.set_codes`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.MultiIndex.set_codes.html#pandas.MultiIndex.set_codes).

### [#](https://www.pypandas.cn/docs/user_guide/advanced.html#创建多级索引和分层索引对象)创建多级索引和分层索引对象

[`MultiIndex`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.MultiIndex.html#pandas.MultiIndex) 对象是[`标准索引对象`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.Index.html#pandas.Index) 的分层模拟，标准索引对象通常将axis标签存储在panda对象中。您可以将`MultiIndex`看作一个元组数组，其中每个元组都是惟一的。可以从数组列表(使用 [`MultiIndex.from_arrays()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.MultiIndex.from_arrays.html#pandas.MultiIndex.from_arrays))、元组数组(使用[`MultiIndex.from_tuples()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.MultiIndex.from_tuples.html#pandas.MultiIndex.from_tuples))或交叉迭代器集(使用[`MultiIndex.from_product()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.MultiIndex.from_product.html#pandas.MultiIndex.from_product))或者将一个 [`DataFrame`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.DataFrame.html#pandas.DataFrame)(使用using [`MultiIndex.from_frame()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.MultiIndex.from_frame.html#pandas.MultiIndex.from_frame))创建多索引。当传递一个元组列表时，`索引`构造函数将尝试返回一个`MultiIndex`。下面的示例演示了初始化多索引的不同方法。
...
当您想要在两个迭代器中对每个元素进行配对时，可以更容易地使用 [`MultiIndex.from_product()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.MultiIndex.from_product.html#pandas.MultiIndex.from_product)方法:
...
还可以使用 [`MultiIndex.from_frame()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.MultiIndex.from_frame.html#pandas.MultiIndex.from_frame)方法直接将一个`DataFrame`对象构造一个`多索引`。这是[`MultiIndex.to_frame()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.MultiIndex.to_frame.html#pandas.MultiIndex.to_frame)的一个补充方法。
...
### [#](https://www.pypandas.cn/docs/user_guide/advanced.html#重构层次标签)重构层次标签

[`get_level_values()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.MultiIndex.get_level_values.html#pandas.MultiIndex.get_level_values)方法将返回特定级别每个位置的标签向量:
...
### [#](https://www.pypandas.cn/docs/user_guide/advanced.html#基本索引轴上的多索引)基本索引轴上的多索引
...
有关如何在更深层次上进行选择，请参见[具有层次索引的横切](https://www.pypandas.cn/docs/user_guide/advanced.html#advanced-xs)。

### [#](https://www.pypandas.cn/docs/user_guide/advanced.html#定义不同层次索引)定义不同层次索引

[`MultiIndex`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.MultiIndex.html#pandas.MultiIndex) 保存了所有被定义的索引层级，即使它们实际上没有被使用。在切割索引时，您可能会注意到这一点。例如:
...
这样做是为了避免重新计算级别，从而使切片具有很高的性能。如果只想查看使用的级别，可以使用[`remove_unused_levels()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.MultiIndex.remove_unused_levels.html#pandas.MultiIndex.remove_unused_levels)方法。
...
若要仅使用已使用的级别来重构`MultiIndex `，可以使用[`remove_unused_levels()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.MultiIndex.remove_unused_levels.html#pandas.MultiIndex.remove_unused_levels)方法。
...
### [#](https://www.pypandas.cn/docs/user_guide/advanced.html#数据对齐和使用-reindex)数据对齐和使用 `reindex`
...
`Series/DataFrames`对象的 [`reindex()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.DataFrame.reindex.html#pandas.DataFrame.reindex) 方法可以调用另一个`MultiIndex` ，甚至一个列表或数组元组:
...
## [#](https://www.pypandas.cn/docs/user_guide/advanced.html#具有层次索引的高级索引方法)具有层次索引的高级索引方法
...
### [#](https://www.pypandas.cn/docs/user_guide/advanced.html#使用切片器)使用切片器

你可以使用多级索引器来切片一个``多级索引

你可以提供任意的选择器，就仿佛你按照标签索引一样，参见[按照标签索引](https://www.pypandas.cn/docs/user_guide/indexing.html#indexing-label), 包含切片，标签构成的列表，标签，和布尔值索引器。
...
你可以使用 [`pandas.IndexSlice`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.IndexSlice.html#pandas.IndexSlice)，即使用`:`，一个更为符合习惯的语法，而不是使用slice(None)。
...
### [#](https://www.pypandas.cn/docs/user_guide/advanced.html#交叉选择)交叉选择

`DataFrame` 的 [`xs()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.DataFrame.xs.html#pandas.DataFrame.xs)方法接受一个额外的参数，从而可以简便地在某个特定的`多级索引`中的某一个层级进行数据的选取。
...
### [#](https://www.pypandas.cn/docs/user_guide/advanced.html#高级重命名索引及对齐)高级重命名索引及对齐

`level`参数已经被加入到pandas对象中的 [`reindex()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.DataFrame.reindex.html#pandas.DataFrame.reindex) 和 [`align()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.DataFrame.align.html#pandas.DataFrame.align) 方法中。这将有助于沿着一个层级来广播值（broadcast values）。例如：
...
### [#](https://www.pypandas.cn/docs/user_guide/advanced.html#使用swaplevel来交换层级)使用`swaplevel`来交换层级

[`swaplevel()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.MultiIndex.swaplevel.html#pandas.MultiIndex.swaplevel)函数可以用来交换两个层级
...
### [#](https://www.pypandas.cn/docs/user_guide/advanced.html#使用reorder-levels来进行层级重排序)使用`reorder_levels`来进行层级重排序

[`reorder_levels()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.MultiIndex.reorder_levels.html#pandas.MultiIndex.reorder_levels)是一个更一般化的 `swaplevel`方法，允许您用简单的一步来重排列索引的层级：
...
### [#](https://www.pypandas.cn/docs/user_guide/advanced.html#对索引和多层索引进行重命名)对`索引`和`多层索`引进行重命名

[`rename()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.DataFrame.rename.html#pandas.DataFrame.rename)方法可以用来重命名`多层索引`，并且他经常被用于`DataFrame`的列名重命名。`renames`的`columns`参数可以接受一个字典，从而仅仅重命名你希望更改名字的列。
...
[`rename_axis()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.DataFrame.rename_axis.html#pandas.DataFrame.rename_axis) 方法可以用于对`Index` 或者 `MultiIndex`进行重命名。尤其的，你可以明确`MultiIndex`中的不同层级的名称，这可以被用于在之后使用 `reset_index()` ，把`多层级索引`的值转换为一个列
...
## [#](https://www.pypandas.cn/docs/user_guide/advanced.html#对多索引进行排序)对`多索引`进行排序
...
在`MultiIndex`上使用 [`is_lexsorted()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.MultiIndex.is_lexsorted.html#pandas.MultiIndex.is_lexsorted) 方法，你可以查看这个索引是否已经被排序。而使用`lexsort_depth` 属性则可以返回排序的深度
...
## [#](https://www.pypandas.cn/docs/user_guide/advanced.html#take方法)Take方法

与`NumPy`的`ndarrays`相似，pandas的 `Index`， `Series`，和`DataFrame` 也提供 [`take()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.DataFrame.take.html#pandas.DataFrame.take) 方法。他可以沿着某个维度，按照给定的索引取回所有的元素。这个给定的索引必须要是一个由整数组成的列表或者ndarray，用以指明在索引中的位置。`take` 也可以接受负整数，作为相对于结尾的相对位置。
...
## [#](https://www.pypandas.cn/docs/user_guide/advanced.html#索引类型)索引类型

我们在前面已经较为深入地探讨过了多层索引。你可以在 [这里](https://www.pypandas.cn/docs/user_guide/timeseries.html#timeseries-overview)，可以找到关于 `DatetimeIndex` 和`PeriodIndex`的说明文件。在 [这里](https://www.pypandas.cn/docs/user_guide/timedeltas.html#timedeltas-index)，你可以找到关于`TimedeltaIndex`的说明。

In the following sub-sections we will highlight some other index types.

下面的一个子章节，我们将会着重探讨另外的一些索引的类型。

### [#](https://www.pypandas.cn/docs/user_guide/advanced.html#分类索引)分类索引

[`CategoricalIndex`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.CategoricalIndex.html#pandas.CategoricalIndex)分类索引 这种索引类型非常适合有重复的索引。这是一个围绕 [`Categorical`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.Categorical.html#pandas.Categorical) 而创建的容器。这可以非常高效地存储和索引的具有大量重复元素的索引。
...
### [#](https://www.pypandas.cn/docs/user_guide/advanced.html#_64位整型索引和范围索引)64位整型索引和范围索引

警告

使用浮点数进行基于数值的索引已经再0.18.0的版本中进行了声明。想查看更改的汇总，请参见 [这里open in new window](https://pandas.pydata.org/pandas-docs/stable/whatsnew/v0.18.0.html#whatsnew-0180-float-indexers)。

[`Int64Index`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.Int64Index.html#pandas.Int64Index) 64位整型索引是pandas中的一种非常基本的索引操作。这是一个不可变的数组组成的一个有序的，可切片的集合。再0.18.0之前，`Int64Index`是会为所有`NDFrame` 对象提供默认的索引。

[`RangeIndex`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.RangeIndex.html#pandas.RangeIndex) 范围索引是`64位整型索引`的子集，在v0.18.0版本加入。现在由范围索引来为所有的`NDFrame`对象提供默认索引。 `RangeIndex`是一个对于 `Int64Index` 的优化版本，能够提供一个有序且严格单调的集合。这个索引与python的 [range typesopen in new window](https://docs.python.org/3/library/stdtypes.html#typesseq-range)是相似的

### [#](https://www.pypandas.cn/docs/user_guide/advanced.html#_64位浮点索引)64位浮点索引

默认情况下，当传入浮点数、或者浮点整型混合数的时候，一个64位浮点索引 [`Float64Index`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.Float64Index.html#pandas.Float64Index)将会自动被建立。这样将能够确保一个存粹而统一的基于标签的索引切片行为，这样`[],ix,loc`对于标量索引和切片的工作行为将会完全一致。
...
### [#](https://www.pypandas.cn/docs/user_guide/advanced.html#间隔索引)间隔索引

*0.20.0中新加入*[`IntervalIndex`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.IntervalIndex.html#pandas.IntervalIndex)和它自己特有的`IntervalDtype`以及 [`Interval`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.Interval.html#pandas.Interval) 标量类型，在pandas中，间隔数据是获得头等支持的。

`IntervalIndex`间隔索引允许一些唯一的索引，并且也是 [`cut()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.cut.html#pandas.cut) 和[`qcut()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.qcut.html#pandas.qcut)的返回类型

#### [#](https://www.pypandas.cn/docs/user_guide/advanced.html#使用间隔索引来进行数据索引)使用`间隔索引`来进行数据索引
...
可以使用[`overlaps()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.IntervalIndex.overlaps.html#pandas.IntervalIndex.overlaps)来创建一个布尔选择器，来选中所有与`给定区间`(`Interval`)重复的所有区间。
...
#### [#](https://www.pypandas.cn/docs/user_guide/advanced.html#使用-cut-和-qcut来为数据分块)使用 `cut` 和 `qcut`来为数据分块

[`cut()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.cut.html#pandas.cut) 和 [`qcut()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.qcut.html#pandas.qcut) 都将返回一个分类`Categorical` 对象，并且每个分块区域都会以 分类索引`IntervalIndex`的方式被创建并保存在它的`.categories`属性中。
...
[`cut()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.cut.html#pandas.cut) 也可以接受一个 `IntervalIndex` 作为他的 `bins` 参数，这样可以使用一个非常有用的pandas的写法。 首先，我们调用 [`cut()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.cut.html#pandas.cut) 在一些数据上面，并且将 `bins`设置为某一个固定的数 ，从而生成bins。

随后，我们可以在其他的数据上调用 [`cut()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.cut.html#pandas.cut)，并传入`.categories` 的值，作为 `bins`参数。这样新的数据就也将会被分配到同样的bins里面
...
#### [#](https://www.pypandas.cn/docs/user_guide/advanced.html#生成一定区间内的间隔)生成一定区间内的间隔

如果我们需要经常地使用步进区间，我们可以使用 [`interval_range()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.interval_range.html#pandas.interval_range) 函数，结合 `start`, `end`, 和 `periods`来建立一个 `IntervalIndex` 对于数值型的间隔，默认的 `interval_range`间隔频率是1，对于datetime类型的间隔则是日历日。
...
`freq` 参数可以被用来明确非默认的频率，并且可以充分地利用各种各样的 [frequency aliases](https://www.pypandas.cn/docs/user_guide/timeseries.html#timeseries-offset-aliases) datetime类型的时间间隔。
...
## [#](https://www.pypandas.cn/docs/user_guide/advanced.html#其他索引常见问题)其他索引常见问题

### [#](https://www.pypandas.cn/docs/user_guide/advanced.html#数值索引)数值索引
...
### [#](https://www.pypandas.cn/docs/user_guide/advanced.html#非单调索引要求严格匹配)非单调索引要求严格匹配

如果一个 `序列` 或者 `数据表`是单调递增或递减的，那么基于标签的切片行为的边界是可以超出索引的，这与普通的python`列表`的索引切片非常相似。索引的单调性可以使用 [`is_monotonic_increasing()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.Index.is_monotonic_increasing.html#pandas.Index.is_monotonic_increasing) 和[`is_monotonic_decreasing()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.Index.is_monotonic_decreasing.html#pandas.Index.is_monotonic_decreasing) 属性来检查
...
`Index.is_monotonic_increasing`和`Index.is_monotonic_decreasing`方法只能进行弱单调性的检查。要进行严格的单调性检查，你可以配合 [`is_unique()`open in new window](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.Index.is_unique.html#pandas.Index.is_unique) 方法一起使用。
...
### [#](https://www.pypandas.cn/docs/user_guide/advanced.html#终止点被包含在内)终止点被包含在内
...
### [#](https://www.pypandas.cn/docs/user_guide/advanced.html#索引会潜在地改变序列的dtype)索引会潜在地改变序列的dtype
...
参见 [this old issueopen in new window](https://github.com/pydata/pandas/issues/2388)了解更详细的讨论过程
